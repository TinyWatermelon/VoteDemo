...
module.exports = {
    async initPage ( ctx ) {
        ...
        const tx_id = queryBody.TxId;
        const title = 'Transaction information page';
		...
		try {
			...
			txresult = await channel.queryTransaction(tx_id);
			txresult = JSON.stringify(txresult);
			blockresult = await channel.queryBlockByTxID(tx_id);
        ...
        await ctx.render('txInfo', {
            title, txresult, blockresult
        })
    }
